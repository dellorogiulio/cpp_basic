...
/*
    ...
    - copy constructor: A copy constructor is constructor whose first parameter is T&‍, const T&‍, volatile T&‍, or const volatile T&‍, and either there are no other parameters (https://en.cppreference.com/w/cpp/language/copy_constructor)
    (for sake of completeness, starting from C++11, - move constructor: https://en.cppreference.com/w/cpp/language/move_constructor which needs more concept to be fully understood)


    It is important to keep in mind one important thing: the compiler has the ability and the freedom to define some constructor for us. 
...
{
    // this function takes a Person by COPY and calls the copy constructor
}

void getAnotherPersonByCopy(AnotherPerson person)
{
    // this function takes an AnotherPerson by COPY and calls the copy constructor
}
